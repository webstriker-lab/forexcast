...
export default function Planner() {
  ...
  // them is meaningless, so both totals are converted to USD first (the
  // same way DebtManager/get_debt_timeline already does for the dedicated
  // debt page), then to the user's chosen display currency.
  const [totalDebtUsd, setTotalDebtUsd] = useState<number | null>(null)
  const [totalSavedUsd, setTotalSavedUsd] = useState<number | null>(null)
  const [totalDebt, setTotalDebt] = useState<number | null>(null)
  const [totalSaved, setTotalSaved] = useState<number | null>(null)

  useEffect(() => {
    getDebtTimeline()
      .then(res => setTotalDebtUsd(res.summary.total_balance))
      .catch(() => setTotalDebtUsd(null))
  }, [debts])
  ...
  useEffect(() => {
    if (totalDebtUsd === null && totalSavedUsd === null) return
    let cancelled = false

    fetchUsdConversionRates([displayCurrency]).then(rates => {
      if (cancelled) return
      setTotalDebt(totalDebtUsd !== null ? convertUsdTo(totalDebtUsd, displayCurrency, rates) : null)
      setTotalSaved(totalSavedUsd !== null ? convertUsdTo(totalSavedUsd, displayCurrency, rates) : null)
    })

    return () => {
      cancelled = true
    }
  }, [totalDebtUsd, totalSavedUsd, displayCurrency])
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Link to="/planner/debts" className="bg-white rounded-lg shadow p-4 hover:shadow-md transition-shadow">
            ...
            <p className="text-2xl font-bold text-red-600">
              {totalDebt !== null ? totalDebt.toLocaleString(undefined, { style: 'currency', currency: displayCurrency }) : '—'}
            </p>
          ...
          <Link to="/planner/goals" className="bg-white rounded-lg shadow p-4 hover:shadow-md transition-shadow">
            <p className="text-sm text-gray-500">Total Saved ({displayCurrency})</p>
            <p className="text-2xl font-bold text-green-600">
              {totalSaved !== null ? totalSaved.toLocaleString(undefined, { style: 'currency', currency: displayCurrency }) : '—'}
            </p>
