...
from app.macro.supabase_rest import upsert_macro_rates, upsert_series
...
def run_series_ingestion(series_map: dict[str, str], table: str) -> int:
    """Generic version of run_macro_ingestion, parameterized by series
    map and destination table -- reused by the three fundamentals below.
    run_macro_ingestion itself keeps its own dedicated implementation
    above, untouched, since it predates this and there's no reason to
    disturb already-stable code for a currently-behavior-identical
    refactor.
    """
    rows = []
    ...
    if not rows and series_map:
        logger.warning(
            "run_series_ingestion(%s) produced zero rows despite %d mapped currencies -- "
            "check FRED_API_KEY and series_map.py",
            table,
            len(series_map),
        )
    upsert_series(table, rows)
    return len(rows)


def run_cpi_ingestion() -> int:
    return run_series_ingestion(CPI_SERIES, "macro_cpi")


def run_gdp_ingestion() -> int:
    return run_series_ingestion(GDP_SERIES, "macro_gdp")


def run_current_account_ingestion() -> int:
    return run_series_ingestion(CURRENT_ACCOUNT_SERIES, "macro_current_account")
