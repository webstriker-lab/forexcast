...
4. **LLM agent** — multi-provider adapter, tool-calling, chat. Shipped (backend only, per its own design doc's scope split — item 6 owns the chat panel UI) — a new `POST /chat` route (the first new backend API route since item 1, justified because chat needs server-side secret handling and multi-step orchestration that a direct Supabase read can't provide), reusing the `llm_settings` table already scaffolded in `0001_init_schema.sql` with PyNaCl sealed-box asymmetric encryption so the frontend can write encrypted keys directly via existing RLS. All seven tools from the Phase 1 spec (`get_forecast`, `get_news_summary`, `get_recommendation`, and user-scoped alert CRUD) are grounded in real Supabase reads/writes, never invented numbers. See [design doc](../specs/2026-08-22-llm-agent-design.md).
5. **Notifications** — Telegram + email in-app linking and dispatch. Consumes the "fired" state item 3's alert evaluation already writes.
   - **5a — Telegram notifications** — Telegram Bot API client, message formatting, Supabase accessors for alert dispatch, job entrypoint (`run_notifications`) with per-currency-skip error isolation, and a 5-minute GitHub Actions cron (`notify.yml`). One-time account linking via `/start <code>` messages polled through Telegram's `getUpdates` offset mechanism. Shipped. See [design doc](../specs/2026-08-22-telegram-notifications-design.md) and [implementation plan](2026-08-22-telegram-notifications.md).
   - **5b — Email notifications** — deferred; layers onto the same dispatch infrastructure once a consumer exists.
6. **Dashboard UI** — graph, chat panel, settings, currency-pair picker. Reads `predictions`/recommendation state directly from Supabase (see item 3's note) unless this task finds a concrete reason a backend API route is actually needed. Shipped — React 18 + Vite + Tailwind CSS SPA with Supabase Auth (email+password + Google OAuth), PairPicker (watchlist CRUD), RateChart (historical rates + prediction overlays with confidence bands), RecommendationCard (ACT NOW/WAIT/VOLATILE), AlertManager (threshold + recommendation_change alerts), ChatPanel (POST /chat with tool-call display), SettingsPage (LLM provider config + Telegram linking), responsive Layout with mobile nav, and PWA manifest. All data reads via Supabase RLS; only backend call is POST /chat (JWT-authenticated). See [design doc](../specs/2026-08-22-dashboard-ui-design.md) and [implementation plan](2026-08-22-dashboard-ui.md).
